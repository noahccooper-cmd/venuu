...
DECLARE
  ...
  -- NEW: weight lookup variables — config-driven multipliers with
  -- hardcoded fallbacks if signal_weights returns NULL for any type.
  v_w_nfc numeric;
  v_w_loyalty numeric;
  v_w_cover numeric;
  v_w_recap numeric;
...
BEGIN
  -- ---- Look up weights from config table with hardcoded fallbacks ----
  SELECT weight INTO v_w_nfc
    FROM signal_weights WHERE signal_type = 'nfc_tap';
  v_w_nfc := COALESCE(v_w_nfc, 1.5);

  SELECT weight INTO v_w_loyalty
    FROM signal_weights WHERE signal_type = 'loyalty_visit';
  v_w_loyalty := COALESCE(v_w_loyalty, 1.0);

  SELECT weight INTO v_w_cover
    FROM signal_weights WHERE signal_type = 'cover_purchase';
  v_w_cover := COALESCE(v_w_cover, 2.0);

  SELECT weight INTO v_w_recap
    FROM signal_weights WHERE signal_type = 'recap_post';
  v_w_recap := COALESCE(v_w_recap, 0.5);

  SELECT weight INTO v_w_direction
    FROM signal_weights WHERE signal_type = 'direction_request';
  v_w_direction := COALESCE(v_w_direction, 0.3);

  SELECT weight INTO v_w_card
    FROM signal_weights WHERE signal_type = 'card_view';
  v_w_card := COALESCE(v_w_card, 0.1);

  SELECT weight INTO v_w_geofence
    FROM signal_weights WHERE signal_type = 'app_open_in_geofence';
  v_w_geofence := COALESCE(v_w_geofence, 1.2);

  SELECT weight INTO v_w_presence
    FROM signal_weights WHERE signal_type = 'background_presence';
  v_w_presence := COALESCE(v_w_presence, 1.5);

  SELECT weight INTO v_w_plan_intent
    FROM signal_weights WHERE signal_type = 'plan_intent';
  v_w_plan_intent := COALESCE(v_w_plan_intent, 0.8);

  -- Read venue context (UNCHANGED from 00039)
  ...
  -- ---- STEP 1: bouncer override (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 2: baseline cascade (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 3 NEW: multipliers from signal_weights config ----
  ...
  -- Multiply counts by config weights (caps preserved from 00039)
  v_user_contrib :=
  ...
  -- ---- STEP 4: estimate_pct -> absolute (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 4.5: delta_pct / expected_pct (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 5: confidence (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 6: state_label (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 7: uncertainty band (UNCHANGED from 00039) ----
  ...
  -- ---- STEP 8: trend + trend_rate (UNCHANGED from 00039) ----
...
BEGIN
  RAISE NOTICE '-----------------------------------';
  RAISE NOTICE 'compute_venue_estimate v3: weights from config';
  RAISE NOTICE 'Multipliers now read from signal_weights table';
  RAISE NOTICE 'Hardcoded fallbacks if config missing';
  RAISE NOTICE '-----------------------------------';
END $$;
